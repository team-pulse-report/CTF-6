...
## Purpose
Keep CTF-6 solvable only along its intended weak-JWT-forgery path: the featured vulnerability stays load-bearing, credentials stay in sync across the Dockerfile and admin store, the build stays reproducible, and the walkthrough honestly documents every shortcut.
